...
# → content-localization:1.8.0-<gpu-arch>  (e.g. :1.8.0-turing)

unset NGC_API_KEY

# Push to your organization's registry (not documented here — use your internal process)
docker push <registry>/<namespace>/content-localization:1.8.0-turing
```
...
docker run --rm --platform linux/amd64 \
  <your-runtime-image>:1.8.0-turing \
...
**GPU architecture:** Build on the **same class** as CAI (e.g. T4 → tag `content-localization:1.8.0-turing`). One image runs on other supported GPUs, but a different arch may spend 5–15 minutes recompiling engines on first start.
...
1. Update [`repo-assembly.json`](runtime/repo-assembly.json): set `image_identifier` to your private registry URI (e.g. `<registry>/<namespace>/content-localization:1.8.0-turing`).
