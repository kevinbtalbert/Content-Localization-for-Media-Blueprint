...
| NIM exits after Triton banner, no `:8004`/`:8005`, `df /dev/shm` shows **64M** | **Raise project shared memory** — `.project-metadata.yaml` sets `shared_memory_limit: 8` (GB) on AMP deploy/redeploy; or Project Settings → Engine → Advanced → **8192** MB manually. LipSync/ASD need ~4–8 GB `/dev/shm` (Docker uses `--shm-size=8g`; CAI default is 64 MB). |
